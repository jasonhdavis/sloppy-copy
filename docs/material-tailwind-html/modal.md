...
[Requires extra Javascript Scripts](/docs/html/modal#required-script)

Use our Tailwind CSS **modal** component to inform users about tasks or
important information that require decisions or involve multiple steps.

A modal is a window that displays critical information, disabling all app
functionality when it appears, and remains on screen until confirmed or
dismissed.
...
Add `data-dialog-target="`{modal}`"` to the trigger element. Add `data-dialog-
backdrop="`{modalName}`"` to the backdrop element. Add `data-
dialog="`{modalName}`"` to the modal element itself.

Make sure that the values for these data attributes are the same.

To close the modal when an element is clicked, add `data-dialog-close="true"`
to that element.
...
This coded example demonstrates a modal component; it includes a trigger
button, a backdrop, and customizable styling for the modal's content and
actions. Upon clicking the "Open Modal" button, the modal is rendered over the
backdrop, ensuring a focused interaction for the user. The content and
...
You can set the size of the modal on your own using the Tailwind CSS classes.
We already made some default sizes for you, check the examples below for
...
This example of a modal component is for connecting a Web 3.0 wallet to a web
application. It features a button to open the modal, a backdrop, wallet
...
modal component. You can pass Tailwind CSS classnames for those data
attributes for animating the modal. Check more about animation data attributes
for modal here.
...
Use the example below to create a modal with a Sign In form, Register form, or
any other form like CRUD examples.
...
your images on Unsplash) and other details.

![nature](https://images.unsplash.com/photo-1485470733090-0aae1788d5af?w=1300&q=80)

![tania
andrew](https://images.unsplash.com/photo-1633332755192-727a05c4013d?w=1300&q=80)
...
---|---  
`data-dialog-target`| Set the modal target element, it should be the same as
the `data-dialog`  
...
Attribute| Description  
---|---  
`data-dialog-backdrop`| Set the modal backdrop element, it should be the same
as the `data-dialog`  
and `data-dialog-target` data attributes.  
`data-dialog-backdrop-close`| Sets whether to close modal when clicking on
...
---|---|---  
`data-dialog`| Set the name of the modal and reference it to the  
...
The modal component needs a required script file to work, you just need to add
the below script file to the bottom of your html file.
...
Check out more modal component examples from [Material Tailwind
