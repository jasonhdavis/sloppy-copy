...
Use our Tailwind CSS Popover to display additional information, context, or
controls related to an item or action. Popovers can be triggered by various
user interactions such as clicking on a button, hovering over a text link, or
focusing on an input field.

Popovers are useful for creating a cleaner, less cluttered user interface by
hiding secondary information or options until they're needed. They're often
used for tooltips, dropdown menus, options menus, form validation messages,
and more.

See below our popover component examples created with HTML and styled with
Tailwind CSS.
...
You can initialize a new popover by adding the `data-popover-
target="`{popoverName}`"` data attribute to the trigger element and the `data-
popover="`{popoverName}`"` to the element that you want to use as the popover.

## Default Popover

The content inside the popover is simple text, styled with `font-sans text-sm
font-normal` and colored with `text-blue-gray-500`. It shows basic usage for
displaying additional information or actions related to the button.
...
You can change the position of the popover relative to it's trigger element by
adding the `data-popover-placement="`{top}`"` data attribute to the popover
...
popover-mount="`{opacity-100}`"`, `data-popover-unmount="`{opacity-0}`"` and
`data-popover-transition="`{transition-opacity}`"` data attributes to the
popover element. You can pass Tailwind CSS class names for those data
attributes for animating the popover.  
...
This example is used to show more information. It contains an image, headline,
...
Material Design. It features multiple React and HTML components, all written
with Tailwind CSS classNamees and Material Design guidelines.

Read More

![image](https://images.unsplash.com/photo-1544928147-79a2dbc1f389?ixlib=rb-4.0.3&ixid=MnwxMjA3fDB8MHxzZWFyY2h8Mnx8dGVhbSUyMGJ1aWxkaW5nfGVufDB8fDB8fA%3D%3D&auto=format&fit=crop&w=800&q=60)
    ...
    <div
      ...
      <div class="flex gap-2">
        <div class="relative h-11 w-full min-w-[200px]">
          <input placeholder="[[email protected]](/cdn-cgi/l/email-protection)"
...
Check out this component example that showcases how to create an interactive
profile information card that appears as a popover. The example includes
several UI elements such as an image, a button, textual information, and
icons, offering a richer user experience.

Profile Info

![tania
andrew](https://images.unsplash.com/photo-1633332755192-727a05c4013d?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1480&q=80)Follow

###### Tania Andrew • @andrew

A skilled developer with a passion for creating innovative solutions and a
proven track record in full-stack development. Adept at working with a variety
of technologies to build robust applications.
...
This popover component is designed to display repository details,
incorporating elements such as links, badges, text descriptions, and icons to
...
@material-tailwind is an easy-to-use components library htmlFor Tailwind CSS
and Material Design.
...
---|---  
`data-popover-target`| Set the popover target element, it should be the same
as the `data-popover` data attribute.  
`data-popover-nested`| Set it to `true` if you want to use one popover inside
another popover.  
...
---|---|---  
`data-popover`| Set the name of the popover and reference it to the  
popover trigger element.|  
`data-popover-offset`| Set the offset between the popover and the popover
trigger element.| 5  
`data-popover-placement`| Set the position of the popover relative to it's
trigger element.| `top`  
`data-popover-mount`| Set the classnaes that should be used when the  
popover is visible.| `opacity-1`  
...
The popover component needs a required script file to work, you just need to
